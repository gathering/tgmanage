...
with open('tg23-koblingsplan.csv', newline='') as csvfile:
    ...
    for row in csv_data:
        ...
        current_iteration['a_interface'] = row[3] if len(row[3].strip()) > 0 else prev_iteration['a_interface']
        ...
        current_iteration['b_interface'] = row[8] if len(row[8].strip()) > 0 else prev_iteration['b_interface']
